            ...
            <div class="card-body pb-0">
                <div class="">
                    <div class="row">
                        ...
                        <div class="col-3 d-flex justify-content-center align-items-center ">
                            <div class="card card-widget widget-user ">
                                <div class="widget-user-header bg-info">
                                    <h3 class="widget-user-username">De: {{ $comunicacion->remitente?->NombreCompleto }}
                                    </h3>
                                </div>
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="widget-user-image">
                                        <img class="img-circle elevation-2"
                                            src="{{ $comunicacion->remitente?->SafeAvatarImg }}" alt="User Avatar">
                                    </div>
                                        ...
                                        @if ($comunicacion->tipo->id == \App\EureLib\Enums\ComunicacionTipoEnum::Aula)
                                            ...
                                            @foreach ($comunicacion->remitente?->profe->MPGs as $MPG)
                                                @if ($MPG->grupo->id == $alumno->grupo->id)
                                                    <span
                                                        class="text-muted text-sm">{{ $MPG->materia->Descripcion }}</span>&nbsp;
                                                @endif
                                            @endforeach
                                        @else
                                        @endif
                                ...
                                <div class="card-body">
                                    @if ($comunicacion->tipo->id == \App\EureLib\Enums\ComunicacionTipoEnum::Aula->value)
                                        ...
                                        @if ($comunicacion->templateDestinatarios == 'CURSO_COMPLETO')
                                            <div class="card d-inline mr-2 mb-2"><span class="p-1">Todo el curso</span>
                                            </div>
                                        @endif
                      ...
                      @endforeach --}}
                                            <div class="card d-inline mr-2 mb-2"><span
                                                    class="p-1">{{ $alumno->nombreYApellido }}</span>
                                            </div>
                                        @endif
                                    @else
                                        {{--                    Mas adelante meter la adminsitrativa? la automática? --}}
                                        @if ($comunicacion->templateDestinatarios == 'TODOS')
                                            <div class="card d-inline mr-2 mb-2"><span class="p-1">Toda la
                                            ...
                                            @foreach ($comunicacion->adjuntos as $adjunto)
                                                <tr style="border-bottom: 1px solid #ddd">
                                                    ...
                                                    <td style="vertical-align: middle" class="p-2">
                                                        <a href="{{ url("/storage/$adjunto->filename") }}"
                                                            target="_blank"><span
                                    ...
                                    <div class="card-body">
                                        ...
                                        @foreach ($comunicacion->comunicacionDestinatarios->where('Cod_Alumno', '=', $alumno->id) as $comDest)
                                            <div class="user-block mb-3">
                                                ...
                                                @else
                                                    <span
                                                        class="description">{{ $comDest->fhLeido->format('d/m/Y H:i') }}</span>
                                                    <span
                                                        class="description font-italic">{{ $comDest->fhLeido->diffForHumans() }}</span>
                                                @endif
                                            </div>
                                        @endforeach
